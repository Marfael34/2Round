...
const Conversation = () => {
  ...
  const [paymentCard, setPaymentCard] = useState({ name: '', number: '', expiry: '', cvc: '' });
  const [shippingAddress, setShippingAddress] = useState({ name: '', street: '', city: '', zip: '', country: 'France' });
  const [selectedRelay, setSelectedRelay] = useState('Mondial Relay - Epicerie du Rond-Point');
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        const res = await securedFetch(`${API_URL}/users?email=${encodeURIComponent(email)}`);
        if (!res.ok) throw new Error("Impossible de charger votre profil.");
        const data = await res.json();
        const userObj = data.member ? data.member[0] : (data['hydra:member'] ? data['hydra:member'][0] : (Array.isArray(data) ? data[0] : data));
        if (!userObj) throw new Error("Utilisateur introuvable.");
        
        setCurrentUser(userObj);
        setShippingAddress({
          name: `${userObj.firstname || ''} ${userObj.lastname || ''}`.trim() || 'Jean Dupont',
        ...
        setPaymentCard(prev => ({
          ...prev,
          name: `${userObj.firstname || ''} ${userObj.lastname || ''}`.trim().toUpperCase() || 'JEAN DUPONT'
        }));
      } catch (err) {
        console.error(err);
  ...
  useEffect(() => {
    ...
    const initConversations = async () => {
      ...
      if (targetProductId) {
        ...
        const existingConv = myConvs.find(c => {
          ...
          return Number(prodId) === Number(targetProductId) && Number(buyerId) === currentUser.id;
        });
        ...
        if (existingConv) {
          ...
          try {
            const prodRes = await securedFetch(`${API_URL}/products/${targetProductId}`);
            if (!prodRes.ok) throw new Error("Produit introuvable.");
            const productData = await prodRes.json();

            const sellerIri = productData.seller?.['@id'] || productData.seller;
            const sellerId = productData.seller?.id || sellerIri?.split('/').pop();
            ...
            const addedConv = refreshed.find(c => c.id === newConv.id) || newConv;
    ...
    initConversations();
  }, [currentUser, targetProductId, initOfferParam, initCheckoutParam, setSearchParams]);
  ...
  const fetchActiveMessages = async (convId, silent = false) => {
    ...
    try {
      const res = await securedFetch(`${API_URL}/messages?conversation=${convId}`);
  ...
  useEffect(() => {
    if (autoCheckout && activeConversation) {
      const accepted = [...messages].reverse().find(m => m.offer && m.offer.status === 'accepted');
      const priceToUse = accepted ? parseFloat(accepted.offer.amount) : parseFloat(activeConversation.product?.price || 0);
  ...
  const handleSubmitOffer = async (e) => {
    ...
    if (isNaN(amount) || amount <= 0 || !activeConversation || !currentUser) return;
  ...
  const handleSubmitCounterOffer = async (e) => {
    ...
    if (isNaN(amount) || amount <= 0 || !activeConversation || !currentUser || !activeOfferToCounter) return;

    const originalPrice = parseFloat(activeConversation.product?.price || 0);
    const minOffer = originalPrice * 0.6;

    if (amount < minOffer) {
      setCounterError(`Votre contre-proposition doit être d'au moins 60% du prix d'origine (soit ${minOffer.toFixed(0)}€).`);
      return;
    }
    if (amount >= originalPrice) {
      setCounterError(`Votre contre-proposition doit être inférieure au prix initial (soit ${originalPrice.toFixed(0)}€).`);
      return;
    }
    ...
    try {
      ...
      const prevOfferId = activeOfferToCounter.id || activeOfferToCounter['@id']?.split('/').pop();
      await securedFetch(`${API_URL}/offers/${prevOfferId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          amount: activeOfferToCounter.amount,
          createdAt: activeOfferToCounter.createdAt,
          status: 'declined'
        })
      });
      ...
      if (!offerRes.ok) throw new Error("Erreur de création de la contre-proposition");
      const offerData = await offerRes.json();
      ...
      if (!msgRes.ok) throw new Error("Erreur d'envoi de la contre-proposition");
  ...
  const handleUpdateOfferStatus = async (offerObj, newStatus) => {
    ...
    try {
      ...
      const sysContent = newStatus === 'accepted' 
  ...
  // 9. Execute Simulated Secure Payment Flow
  const handleConfirmSimulatedPayment = () => {
    ...
    if (!paymentCard.name || !paymentCard.number || !paymentCard.expiry || !paymentCard.cvc) {
    ...
    // Simulate payment authorization updates for immersive feedback
    const loadingPhases = [
      { delay: 0, text: 'Initialisation du paiement sécurisé 3D Secure...' },
      { delay: 800, text: 'Vérification de l\'adresse de livraison...' },
      { delay: 1600, text: 'Demande d\'autorisation auprès de la banque...' },
      { delay: 2400, text: 'Paiement validé ! Enregistrement de la commande...' }
    ];

    loadingPhases.forEach(phase => {
      setTimeout(() => {
        setCheckoutLoadingText(phase.text);
      }, phase.delay);
    });
    ...
    setTimeout(async () => {
      try {
        ...
        await securedFetch(`${API_URL}/messages`, {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            content: `L'article a été acheté ! Montant payé : ${checkoutAmount.toFixed(2)}€ (Livré à ${shippingAddress.city})`,
            isRead: false,
            createdAt: new Date().toISOString(),
            users: `/api/users/${currentUser.id}`,
            conversation: `/api/conversations/${activeConversation.id}`
        ...
        console.error(err);
        setCheckoutStep('form');
        alert("Erreur lors de la validation finale de l'achat.");
      }
    }, 3200);
  };
  ...
  // Active user role
  const activeBuyerId = activeConversation?.buyer?.id || activeConversation?.buyer?.split('/').pop();
  const isBuyer = activeConversation && currentUser && Number(activeBuyerId) === currentUser.id;

  // Scan messages to check if product has already been sold
  const isProductSold = messages.some(msg => msg.content && msg.content.includes("L'article a été acheté"));



  // Find the latest accepted offer, if any
  const acceptedOfferMessage = [...messages].reverse().find(m => m.offer && m.offer.status === 'accepted');
  const acceptedOffer = acceptedOfferMessage ? acceptedOfferMessage.offer : null;

  // Original product details
  const productPrice = parseFloat(activeConversation?.product?.price || 0);
  const activePrice = acceptedOffer ? parseFloat(acceptedOffer.amount) : productPrice;
    ...
    <div className="min-h-screen bg-black text-white font-inter flex flex-col">
      <div className="max-w-[1200px] w-full mx-auto px-4 md:px-8 py-6 flex-1 flex flex-col h-[calc(100vh-80px)]">
        ...
        <div className="flex-1 flex border border-white/10 bg-[#070707] overflow-hidden rounded-sm relative">
          ...
          <div className={`w-full md:w-1/3 border-r border-white/10 flex flex-col shrink-0 ${activeConversation ? 'hidden md:flex' : 'flex'}`}>
            <div className="p-4 border-b border-white/10 bg-black/40">
              <span className="text-xs text-gray-500 font-bold uppercase tracking-wider">Discussions</span>
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-12 h-12 rounded-full bg-[#151515] border border-white/10 flex items-center justify-center overflow-hidden shrink-0">
                        {otherUser.avatar ? (
                          <img src={otherUser.avatar} alt={otherUser.pseudo} className="w-full h-full object-cover" />
                        ) : (
          ...
          {/* 2. Active Chat Area */}
          <div className={`flex-1 flex flex-col min-w-0 ${!activeConversation ? 'hidden md:flex justify-center items-center p-8 text-center' : 'flex'}`}>
            {activeConversation ? (
              <>
                  ...
                  <div className="flex items-center gap-4 min-w-0">
                    ...
                    <div className="w-14 h-14 border border-white/10 overflow-hidden rounded-sm bg-[#111] shrink-0">
                      {getProductImage(activeConversation.product) ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-xs text-gray-600">Pas d'image</div>
                      )}
                    </div>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-gray-400 font-light mt-0.5">
                        Vendeur : <span className="font-semibold">{getParticipant(activeConversation)?.pseudo}</span>
                      </p>
                      
                      <div className="flex items-center gap-2 mt-1">
                        {acceptedOffer ? (
                          <>
                            <span className="text-xs text-gray-500 line-through">{productPrice.toFixed(2)}€</span>
                            <span className="text-sm font-bold text-green-500">{acceptedOffer.amount.toFixed(2)}€ (Négocié)</span>
                          </>
                        ) : (
                          <span className="text-sm font-bold text-red-500">{productPrice.toFixed(2)}€</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-linear-to-b from-transparent to-black/10">
                  {loadingMessages ? (
                    ...
                    messages.map((msg) => {
                      const senderId = msg.users?.id || msg.users?.split('/').pop();
                              ...
                              <div className="flex items-center justify-between mb-4 pb-2.5 border-b border-white/5">
                                <div className="flex items-center gap-2 text-gray-300">
                                  ...
                                  <span className="text-[10px] font-bold uppercase tracking-widest">
                                    {isOfferFromBuyer ? "Offre de l'acheteur" : "Contre-offre du vendeur"}
                                  </span>
                                </div>
                                <span className={`text-[9px] font-bold uppercase tracking-wider py-0.5 px-2 rounded-sm border ${badgeColor}`}>
                                  {badgeText}
                                </span>
                              </div>

                              <div className="flex items-baseline gap-2">
                                <span className="text-3xl font-bebas tracking-wide text-white">{offerObj.amount}€</span>
                              ...
                              <div className="mt-5">
                                {isOfferPending ? (
                                  !isOfferSender ? (
                                    ...
                                    <div className="flex flex-col gap-2">
                                      <div className="flex gap-2">
                                        <button 
                                          onClick={() => handleUpdateOfferStatus(offerObj, 'accepted')}
                                        ...
                                        <button 
                                          onClick={() => handleUpdateOfferStatus(offerObj, 'declined')}
                                      ...
                                      >
                                        <FaRotateRight /> Faire une contre-proposition
                                      </button>
                                    </div>
                                  ) : (
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-gray-400 py-1.5">
                                      ...
                                      <span>En attente de réponse de l'autre membre</span>
                                    </div>
                                  )
                                  ...
                                  isBuyer ? (
                                    ...
                                    >
                                      <FaCreditCard /> Finaliser l'achat - {offerObj.amount}€
                                    </button>
                                  ) : (
                      ...
                      const isPurchaseMessage = msg.content && msg.content.includes("L'article a été acheté");
                      ...
                      return (
                        <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                          <div className={`max-w-[75%] p-3.5 rounded-2xl text-xs md:text-sm leading-relaxed shadow-md ${isMe ? 'bg-red-600 text-white rounded-br-none' : 'bg-neutral-800 border border-white/5 text-gray-200 rounded-bl-none'}`}>
                            {msg.content}
                          </div>
                          {/* Time */}
                    ...
                    <div className="h-full flex items-center justify-center text-gray-500 text-xs py-12">
                      Début de la conversation. Envoyez un message ou proposez une offre.
                    </div>
                  )}
                ...
                <div className="p-4 border-t border-white/10 bg-black/40 flex flex-col gap-3 shrink-0">
                  <form onSubmit={handleSendMessage} className="flex gap-2">
                    {/* Proposer une offre tag button (Only show to buyer if product not sold) */}
                    ...
                    <input
                      ...
                      className="flex-1 bg-black border border-white/10 focus:border-red-600 outline-none rounded-sm px-4 text-sm text-white placeholder-gray-600 transition-colors"
                    />
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <h3 className="font-bebas text-2xl uppercase tracking-wider">Vos discussions</h3>
                <p className="text-gray-500 text-sm max-w-xs">
                  Sélectionnez une discussion à gauche pour négocier le prix ou discuter avec un membre.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#0a0a0a] border border-white/15 rounded-lg p-6 md:p-8 shadow-2xl relative">
            ...
            <p className="text-xs text-gray-400 mb-6 leading-relaxed">
              ...
              <span className="text-white font-bold">{activeConversation.product?.title}</span>. Le prix d'origine est de{' '}
              <span className="text-red-500 font-bold">{productPrice}€</span>.
            </p>

            {/* Quick Discount Recommendations */}
            <div className="mb-6">
              <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-3">Contre-propositions rapides</span>
            ...
            <form onSubmit={handleSubmitOffer} className="space-y-5">
              <div>
                ...
                <span className="block text-[9px] text-gray-500 mt-2 italic">
                  Note: Par respect des règles anti-spam Vinted, vous ne pouvez pas proposer une réduction supérieure à 40% du prix d'origine.
                </span>
              </div>
      ...
      {showCounterModal && activeConversation && activeOfferToCounter && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#0a0a0a] border border-white/15 rounded-lg p-6 md:p-8 shadow-2xl relative">
            ...
            <p className="text-xs text-gray-400 mb-6 leading-relaxed">
              L'offre actuelle de l'autre membre est de <span className="text-white font-bold">{activeOfferToCounter.amount}€</span>.
              Proposez un prix intermédiaire pour l'article <span className="text-white font-bold">{activeConversation.product?.title}</span> (Prix initial: <span className="text-red-500 font-bold">{productPrice}€</span>).
            </p>

            {/* Quick Discount Recommendations */}
            <div className="mb-6">
              <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-3">Valeurs suggérées</span>
              <div className="flex gap-3">
                {getDiscountSuggestions(productPrice).map((sugg, i) => (
                  <button
                    key={i}
                  ...
                  >
                    <span className="text-orange-400 font-bold">{sugg.pct}</span>
                    <span className="text-[10px] text-gray-400">{sugg.val}€</span>
                  </button>
                ))}
              </div>
            ...
            <form onSubmit={handleSubmitCounterOffer} className="space-y-5">
              ...
              >
                {sendingCounter ? 'Soumission...' : "Envoyer la contre-proposition"}
              </button>
            </form>
          </div>
        </div>
          ...
          <div className="w-full max-w-lg bg-[#0c0c0c] border border-white/10 rounded-xl p-6 md:p-8 shadow-2xl relative my-8">
              ...
              <>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-4 p-3 bg-neutral-900/40 border border-white/5 rounded-md">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-sm text-white truncate">{activeConversation.product?.title}</h4>
                      <p className="text-xs text-gray-400 mt-1 uppercase font-semibold">Taille: {activeConversation.product?.size || 'Unique'}</p>
                      <p className="text-xs text-gray-500 mt-0.5">Vendu par: {getParticipant(activeConversation)?.pseudo}</p>
                    </div>
                  </div>

                  {/* Pricing Details */}
                  <div className="space-y-2.5 text-xs text-gray-300">
                    <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1">Détails de la transaction</span>
                    <div className="flex justify-between">
                      <span>Prix de l'article</span>
                      <span className="font-semibold text-white">
                        {acceptedOffer ? (
                          <>
                            <span className="line-through text-gray-500 mr-1.5">{productPrice.toFixed(2)}€</span>
                            <span className="text-green-500 font-bold">{checkoutAmount.toFixed(2)}€</span>
                          </>
                        ) : (
                    ...
                    <div className="flex justify-between">
                      <span className="flex items-center gap-1">
                        ...
                        <FaShieldHalved className="text-[10px] text-emerald-500" title="Garantie remboursement 2Round" />
                      </span>
                      <span className="text-white">{(0.70 + (checkoutAmount * 0.05)).toFixed(2)}€</span>
                    </div>
                    ...
                    <div className="flex justify-between text-base font-bold text-white uppercase tracking-wider">
                      ...
                      <span className="text-red-500 text-lg">
                        {(checkoutAmount + (0.70 + (checkoutAmount * 0.05)) + 2.88).toFixed(2)}€
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold">1. Point Relais Mondial Relay</span>
                    <select
                      value={selectedRelay}
                    ...
                    >
                      <option value="Mondial Relay - Epicerie du Rond-Point">Mondial Relay - Epicerie du Rond-Point (2 Rue de la Marne, Paris)</option>
                      <option value="Relais Colis - Tabac de la Place">Relais Colis - Tabac de la Place (15 Place de la République, Paris)</option>
                      <option value="Mondial Relay - Supermarché U">Mondial Relay - Supermarché U (88 Boulevard Sébastopol, Paris)</option>
                    </select>
                  </div>

                  {/* Delivery Address Details */}
                  <div className="space-y-3">
                    <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold">2. Adresse de Facturation</span>
                  ...
                  {/* Credit Card Inputs */}
                  <div className="space-y-3">
                    <span className="block text-[10px] text-gray-500 uppercase tracking-widest font-bold">3. Carte de Crédit</span>
                    <div className="space-y-2">
                      <input
                        ...
                        onChange={(e) => setPaymentCard({...paymentCard, name: e.target.value.toUpperCase()})}
                        className="w-full bg-black border border-white/10 focus:border-red-600 outline-none rounded-md p-3 text-xs text-white uppercase tracking-wider"
                      />
                      <div className="relative">
                        <input
                          type="text"
                          ...
                          onChange={(e) => setPaymentCard({...paymentCard, number: e.target.value.replace(/\D/g, '')})}
                          className="w-full bg-black border border-white/10 focus:border-red-600 outline-none rounded-md p-3 pr-10 text-xs text-white tracking-widest"
                        />
                        <FaCreditCard className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500" />
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <input
                          ...
                          className="w-full bg-black border border-white/10 focus:border-red-600 outline-none rounded-md p-3 text-xs text-white tracking-widest"
                        />
                        <input
                          type="text"
                          ...
                          onChange={(e) => setPaymentCard({...paymentCard, cvc: e.target.value.replace(/\D/g, '')})}
                          className="w-full bg-black border border-white/10 focus:border-red-600 outline-none rounded-md p-3 text-xs text-white tracking-widest"
                        />
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 uppercase tracking-widest rounded-md text-xs transition-colors cursor-pointer flex items-center justify-center gap-2 mt-4 shadow-lg shadow-emerald-950/20"
                  >
                    <FaShieldHalved /> Confirmer et Payer {(checkoutAmount + (0.70 + (checkoutAmount * 0.05)) + 2.88).toFixed(2)}€
                  </button>
                </div>
              </>
            )}
            ...
            {checkoutStep === 'loading' && (
              <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
                <FaCircleNotch className="text-5xl text-emerald-500 animate-spin" />
                <h4 className="font-bebas text-3xl uppercase tracking-wider text-white mt-4">
                  Traitement du paiement
                </h4>
                <p className="text-xs text-gray-400 max-w-xs transition-all duration-300 font-medium">
                  {checkoutLoadingText || "Chargement..."}
                </p>
                <div className="w-48 h-1.5 bg-neutral-900 rounded-full overflow-hidden mt-2 border border-white/5">
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 max-w-xs mt-3 leading-relaxed">
                    Votre commande pour <span className="text-white font-bold">{activeConversation.product?.title}</span> a été validée avec succès.
                  </p>
                  <p className="text-xs text-gray-500 max-w-xs mt-2">
                    Le Point Relais sélectionné est : <br />
                    <span className="text-white font-medium">{selectedRelay}</span>
                  </p>
                </div>
                ...
                <div className="bg-neutral-900 border border-white/5 p-4 rounded-md w-full text-left space-y-2">
                  ...
                  <div className="flex justify-between text-[10px] text-gray-400 uppercase font-bold tracking-wider">
                    ...
                    <span className="text-emerald-400">{(checkoutAmount + (0.70 + (checkoutAmount * 0.05)) + 2.88).toFixed(2)}€</span>
                  </div>
                </div>
