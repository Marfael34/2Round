import { FiTrash2, FiMessageSquare, FiShield, FiXCircle } from "react-icons/fi";

const ConversationReports = ({ reports, handleDeleteReport, handleDismissReport, handleOpenSanctionModal }) => {
  return (
          ...
          {reports.length === 0 ? (
            <tr>
              <td colSpan="6" className="py-8 text-center text-gray-500">
                Aucun signalement de conversation trouvé.
              </td>
            </tr>
          ) : (
            reports.map((report) => {
              ...
              if (!targetPseudo && report.conversation) {
              ...
              const candidates = [
                ...
                { role: "Interlocuteur", id: targetId, pseudo: targetPseudo }
              ].filter(c => c.id);
              targetPseudo = targetPseudo || "Inconnu";
                  ...
                  <td className="py-4 px-4 text-right">
                    <div className="flex justify-end gap-2">
                      {report.conversation?.id && (
                        <button
                          onClick={() => {
                            window.location.href = `/conversation?conversationId=${report.conversation.id}`;
                          }}
